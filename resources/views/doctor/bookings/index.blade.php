...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">{{ __('My Bookings') }}</h2>
    </div>
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-6">
        <form action="{{ route('doctor.bookings.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <!-- Status Filter -->
            <div>
                <select name="status" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" onchange="this.form.submit()">
            ...
            <!-- Date Filter -->
            <div>
                <select name="date_filter" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" onchange="this.form.submit()">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($bookings as $booking)
                <a href="{{ route('doctor.bookings.show', $booking) }}" class="block group">
                    <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 border border-gray-100 h-full flex flex-col">
                        <div class="p-5 flex-1">
                            ...
                            <div class="flex items-center mb-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500">
                                        {{ $booking->appointment_date->format('M d, Y') }} • {{ $booking->appointment_time->format('h:i A') }}
                                    </p>
                                </div>
                            </div>
                            ...
                            @if($booking->notes)
                                <p class="text-sm text-gray-600 line-clamp-2 mb-2">
                                    <span class="font-medium text-gray-700">{{ __('Note:') }}</span> {{ $booking->notes }}
                                </p>
                            @endif
                        </div>
                        <div class="bg-gray-50 px-5 py-3 border-t border-gray-100 flex justify-between items-center">
                             <span class="text-sm font-medium text-gray-600">
                                {{ __('Price:') }} ${{ number_format($booking->price_at_booking, 2) }}
                             </span>
    ...
    @else
        <div class="text-center py-12 bg-white rounded-lg shadow-sm">
            ...
            <p class="mt-1 text-sm text-gray-500">{{ __('Adjust your filters or search to find what you are looking for.') }}</p>
        </div>
    @endif
</div>
@endsection
