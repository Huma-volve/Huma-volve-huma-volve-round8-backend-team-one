            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100 p-6">
                    ...
                    <div class="flex items-start">
                        <div
                            class="h-20 w-20 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-500 font-bold text-3xl mr-6 uppercase flex-shrink-0">
                        ...
                        <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label
                                    class="block text-xs text-gray-400 uppercase font-bold">{{ __('Birthdate') }}</label>
                ...
                <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100 p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label
                                class="block text-xs text-gray-400 uppercase font-bold">{{ __('Payment Status') }}</label>
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                        ...
                        <div class="md:col-span-2">
                            ...
                            <p class="text-gray-800 bg-gray-50 p-3 rounded-md mt-1">
                                {!! nl2br(e($booking->notes ?? __('No notes provided.'))) !!}
                            </p>
                        </div>
                        ...
                        @if ($booking->status === 'cancelled')
                            <div class="md:col-span-2 bg-red-50 p-4 rounded-md border border-red-100 mt-2">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label
                                            class="block text-xs text-red-400 uppercase font-bold">{{ __('Cancelled At') }}</label>
                                        <p class="text-red-900 text-sm">
                                            {{ $booking->cancelled_at ? $booking->cancelled_at->format('M d, Y h:i A') : '-' }}
                                        </p>
                                    </div>
                                    <div>
                                        <label
                                            class="block text-xs text-red-400 uppercase font-bold">{{ __('Cancelled By') }}</label>
                                        <p class="text-red-900 text-sm">{{ $booking->cancelledBy?->name ?? 'System/User' }}
                                        </p>
                                    </div>
                                    <div class="md:col-span-2">
                                        <label
                                            class="block text-xs text-red-400 uppercase font-bold">{{ __('Reason') }}</label>
                ...
                @if ($booking->status !== 'cancelled' && $booking->status !== 'completed')
                    ...
                    <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100 p-6">
                        ...
                        <div x-data="{ showCancel: false, showReschedule: false }">
                            ...
                            <!-- Reschedule Form -->
                            <div x-show="showReschedule" x-transition class="mt-4 pt-4 border-t" x-data="{
                                selectedDate: '',
                                availableTimes: [],
                                slots: {{ json_encode($groupedSlots) }},
                                updateTimes() {
                                    this.availableTimes = this.slots[this.selectedDate] || [];
                                    $refs.timeSelect.value = ''; // Reset time select
                                }
                            }">
                                ...
                                <form action="{{ route('doctor.bookings.reschedule', $booking) }}" method="POST">
                                    ...
                                    <div class="mb-3">
                                        <label
                                            class="block text-xs font-medium text-gray-700 mb-1">{{ __('Select Date') }}</label>
                                        <select x-model="selectedDate" @change="updateTimes()"
                                            ...
                                            <template x-for="(daySlots, date) in slots" :key="date">
                                                <option :value="date"
                                                    x-text="date + ' (' + daySlots[0].day_name + ')'"></option>
                                            </template>
                                        </select>
                                        <input type="hidden" name="appointment_date" x-bind:value="selectedDate">
                                    </div>

                                    <!-- Time Selection -->
                                    <div class="mb-3" x-show="selectedDate">
                                        <label
                                            class="block text-xs font-medium text-gray-700 mb-1">{{ __('Select Time') }}</label>
                                        <select name="appointment_time" x-ref="timeSelect"
                                            class="block w-full text-sm border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                            required>
                                            <option value="">{{ __('Choose a time...') }}</option>
                                            <template x-for="slot in availableTimes" :key="slot.start_time">
                                                <option :value="slot.start_time" x-text="slot.start_time"></option>
                                            </template>
                                        </select>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        <label
                                            class="block text-xs font-medium text-gray-700 mb-1">{{ __('Rescheduling Note (Optional)') }}</label>
                                        <textarea name="reschedule_note" rows="2"
                                            class="block w-full text-sm border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                            placeholder="{{ __('Reason for rescheduling or extra details...') }}"></textarea>
                                    </div>

                                    <button type="submit"
                                        class="w-full inline-flex justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-indigo-700 bg-indigo-100 hover:bg-indigo-200"
                            ...
                            <div x-show="showCancel" x-transition class="mt-4 pt-4 border-t">
                                ...
                                <form action="{{ route('doctor.bookings.cancel', $booking) }}" method="POST">
                                    ...
                                    <div class="mb-3">
                                        <label for="reason"
                                            class="block text-xs font-medium text-gray-700 mb-1">{{ __('Reason for Cancellation') }}</label>
                                        <textarea id="reason" name="cancellation_reason" rows="3"
                                    ...
                                    <button type="submit"
                                        class="w-full inline-flex justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700">
