    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div>
            ...
            <div class="mt-2 flex items-center gap-4">
                ...
                @else
                    <div
                        class="h-20 w-20 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 text-sm">
                        No Photo
                    </div>
                @endif
        ...
        <div>
            ...
            <x-text-input id="bio" name="bio" type="text" class="mt-1 block w-full"
                value="{{ old('bio', $user->doctorProfile->bio ?? '') }}" autofocus autocomplete="name" />
        ...
        <div>
            ...
            <x-text-input id="clinic_address" name="clinic_address" type="text" class="mt-1 block w-full"
                :value="old('clinical-address', $user->doctorProfile->clinic_address ?? '')" />
        ...
        <div>
            ...
            <x-text-input id="session_price" name="session_price" type="number" step='0.01'
                class="mt-1 block w-full" :value="old('session_price', $user->doctorProfile->session_price ?? '')" />
        ...
        <div>
            ...
            <x-text-input id="license_number" name="license_number" type="text" class="mt-1 block w-full"
                :value="old('license_number', $user->doctorProfile->license_number ?? '')" />
            <x-input-error class="mt-2" :messages="$errors->get('license_number')" />
        </div>

        <div>
            <x-input-label for="speciality" :value="__('Speciality')" />
            <x-text-input id="speciality" name="speciality" type="text" class="mt-1 block w-full"
                value="{{ $user->doctorProfile->speciality->name ?? '' }}" readonly />
        ...
        <div class="flex items-center gap-4">
            ...
            @if (session('status') === 'profile-updated')
                <p x-data="{ show: true }" x-show="show" x-transition.opacity.duration.500ms x-init="setTimeout(() => show = false, 2000)"
